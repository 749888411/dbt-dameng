...
INSERT ALL
    INTO {schema}.seed ("_user_id", "user name", "birth date in yyyy-mm-dd", income, last_login_date, "desc") VALUES 
...
# seeds/add_column.sql
add_column_sql = """
-- Add a new column
ALTER TABLE {schema}.seed ADD ("birth date in yyyy-mm-dd" DATE )

"""

# seeds/drop_column.sql
drop_column_sql = """
-- Drop a column
ALTER TABLE {schema}.seed DROP ("birth_ date in yyyy-mm-dd") CASCADE CONSTRAINTS
"""
...
class TestSyncSchemaIncrementalMergeQuotedColumns:
    ...
    def seeds(self):
        return {
            ...
            "add_column.sql": add_column_sql,
            "drop_column.sql": drop_column_sql
        }
    ...
    def test_run_dbt(self, project):
        ...
        merge into dbt_test.my_incr_model target
          using o$pt_my_incr_model175348 temp
          on (
        ...
        when matched then
          ...
          target.BIRTH_DATE = temp.BIRTH_DATE,
        ...
        project.run_sql_file(Path("seeds") / Path("add_column.sql"))
        project.run_sql_file(Path("seeds") / Path("drop_column.sql"))
